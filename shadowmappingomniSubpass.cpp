...
public:
	...
	~VulkanExample()
	{
		...
		// Depth attachment
		for (auto view : offscreenPass.depth.view)
			vkDestroyImageView(device, view, nullptr);

		vkDestroyImageView(device, offscreenPass.depth.readView, nullptr);
	...
	{
		std::array<VkAttachmentDescription, FACE_COUNT> attchmentDescriptions ;
		for (auto& attchmentDescription : attchmentDescriptions)
		{
			...
			attchmentDescription.loadOp			= VK_ATTACHMENT_LOAD_OP_DONT_CARE;// VK_ATTACHMENT_LOAD_OP_CLEAR;						// Clear depth at beginning of the render pass
			...
			attchmentDescription.initialLayout  = VK_IMAGE_LAYOUT_UNDEFINED;// VK_IMAGE_LAYOUT_UNDEFINED;					// We don't care about initial layout of the attachment
			attchmentDescription.finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;//VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL ;	// Attachment will be transitioned to shader read at render pass end
			attchmentDescription.flags = 0;
		}
		...
		for (int face = 0; face < FACE_COUNT; face++)
		{
			...
			// from current to next everything should stay the same
			dependency.srcSubpass = face;
			dependency.dstSubpass = face+1;
		...
		renderPassCreateInfo.attachmentCount = attchmentDescriptions.size();
	...
	void prepareOffscreenFramebuffer()
	{
		...
		offscreenPass.depth.view.resize(FACE_COUNT);
		for (int face = 0; face < FACE_COUNT; face++)
		{
			VkImageViewCreateInfo depthStencilView = vkTools::initializers::imageViewCreateInfo();
			depthStencilView.viewType = VK_IMAGE_VIEW_TYPE_2D_ARRAY; //_ARRAY
			...
			VK_CHECK_RESULT(vkCreateImageView(device, &depthStencilView, nullptr, &offscreenPass.depth.view[face]));
		...
		depthStencilView.image = offscreenPass.depth.image;
		VK_CHECK_RESULT(vkCreateImageView(device, &depthStencilView, nullptr, &offscreenPass.depth.readView));
		...
		fbufCreateInfo.attachmentCount = FACE_COUNT;
		fbufCreateInfo.pAttachments = offscreenPass.depth.view.data();
		fbufCreateInfo.width = offscreenPass.width;
		fbufCreateInfo.height = offscreenPass.height;
		fbufCreateInfo.layers = 1; FACE_COUNT;// 1;
	...
	void buildOffscreenCommandBuffer()
	{
		...
		std::array<VkClearValue, FACE_COUNT> clearValues;
	 ...
	 // renderPassBeginInfo.pClearValues = clearValues.data();
		...
		for (int face = 0; face < FACE_COUNT; face++)
		{
			...
			clearAttachment.clearValue = VkClearValue{ 1.0,0 };

			VkClearRect rect;
			rect.baseArrayLayer = face;
			rect.layerCount = 1;
			rect.rect = VkRect2D{ {0,0},{ SHADOWMAP_DIM ,SHADOWMAP_DIM } };
		 
			vkCmdClearAttachments(offscreenPass.commandBuffer,1, &clearAttachment, 1, &rect);
			...
			if(face < (FACE_COUNT - 1))
			 	vkCmdNextSubpass(offscreenPass.commandBuffer, VK_SUBPASS_CONTENTS_INLINE);
		}
